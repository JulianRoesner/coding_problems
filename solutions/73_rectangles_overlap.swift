...
func overlap(of : Rectangle, with: Rectangle) -> Bool{
	let ofInWithHorizontical = of.x > with.x && of.y < with.x + with.width  
	let withInOfHorizontical = with.x > of.x && with.x < of.x + of.width 
	let ofInWithVertical = of.y > with.y && of.y < with.y + with.height 
	let withInOfVertical = with.y > of.y && with.y < of.y + of.height 
	
	return (ofInWithHorizontical && ( ofInWithVertical || withInOfVertical)) || (withInOfHorizontical && ( ofInWithVertical || withInOfVertical))
}
