...
class BinarySearchTree: CustomStringConvertible{
	...
	public func traversePostorder() -> [Int]{
		...
		var searchThrough : [Node] = Array()
		var resultStack : [Node] = Array()
		var result : [Int] = Array()
		searchThrough.append(current)
		while (searchThrough.count > 0){
			current = searchThrough.popLast()!
			if current.leftChild != nil{
				searchThrough.append(current.leftChild!)
			}
			if current.rightChild != nil{
				searchThrough.append(current.rightChild!)
			}
			resultStack.append(current)
		}

		while (resultStack.count > 0){
			current = resultStack.popLast()!
